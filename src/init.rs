...
fn run_wizard() -> Result<WizardConfig, RumError> {
    ...
    let (image_url, image_comment) = prompt_os_image()?;
    let (cpus, memory_mb) = prompt_resources()?;
    let hostname = prompt_hostname()?;
    let (nat, interfaces) = prompt_network()?;
    let mounts = prompt_mounts()?;
    let (drives, filesystems) = prompt_storage()?;
